...
betCancelKeyword = ["anuluj", "cancelbet", "anulujzaklad", "anulujbet"]
betHelpKeyword = ["jaksiezakladac", "bethelp", "jakzakladac", "zakladyporadnik", "betporadnik"]
betPlacingGuideKeyword = ["jakobstawiac", "jakobstawic", "obstawianiehelp", "obstawianieporadnik"]
betWinningsKeyword = ["ilewygram", "iledostane", "mojawygrana", "potentialwin"]
betMyBetsKeyword = ["mojezaklady", "mybets", "mojezakłady"]
betCreationGuideKeyword = ["jakstworzyczaklad", "jakstworzyc", "tworzenieguide", "jakstworzycbet"]
betCommandsKeyword = ["zakladyhelp", "betcommands", "zakladykomendy", "bethelper"]
    ...
    if len(commands) > 1:
        ...
        #find proper pub to meet together (NEEDS WORK)
            ...
            if len(commands) == 2:
                ...
                returnText = "Uzyj: !anulujzaklad <id_zakladu>\nPrzyklad: !anulujzaklad 0"
        ...
        elif commands[0] in betWinningsKeyword:
            if len(commands) == 2:
                if str(commands[1]).isdigit():
                    bet_id = int(commands[1])
                    result, bet, error = betting.calculatePotentialWinnings(str(author), bet_id)
                    if error:
                        returnText = error
                    else:
                        returnEmbed = embedgen.generatePotentialWinnings(result, user['name'])
                else:
                    returnText = "ID zakladu musi byc liczba!"
            else:
                returnText = "Uzyj: !ilewygram <id_zakladu>\nPrzyklad: !ilewygram 0"
        ...
        if message in betPlacingGuideKeyword:
            returnEmbed = embedgen.generateBetPlacingGuide()

        if message in betMyBetsKeyword:
            user_bets = betting.getUserBets(str(author))
            user = db.retrieveUser('discord_id', str(author))
            username = user['name'] if user else "Nieznany"
            returnEmbed = embedgen.generateUserBets(user_bets, username)

        if message in betCreationGuideKeyword:
            returnEmbed = embedgen.generateBetCreationGuide()

        if message in betCommandsKeyword:
            returnEmbed = embedgen.generateBetCommands()
